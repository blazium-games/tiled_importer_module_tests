...
func test_tileson_parser_validity() -> void:
    # Validate the TiledTileson endpoint
    var tson = TiledTileson.new()
    assert_not_null(tson, "Failed to instantiate TiledTileson")

    # Parse an existing json file
    var map: TiledMap = tson.parse_file("res://tests/data/test-maps/simple_map.json")
    ...
    var first_ts: TiledTileset = tilesets[0]
...
func test_tileson_objects_and_chunks() -> void:
    ...
    for layer in map.get_layers():
        ...
        for obj in objects:
            assert_not_null(obj, "TiledObject should be properly formed")
...
func test_tileson_remaining_classes() -> void:
    var grid := TiledGrid.new()
    assert_not_null(grid, "TiledGrid bound")
    var terrain := TiledTerrain.new()
    assert_not_null(terrain, "TiledTerrain bound")
    var text := TiledText.new()
    assert_not_null(text, "TiledText bound")
    ...
    var transf := TiledTransformations.new()
    assert_not_null(transf, "TiledTransformations bound")
    ...
    var tclass := TiledClass.new()
    assert_not_null(tclass, "TiledClass bound")
    var enumdef := TiledEnumDefinition.new()
    assert_not_null(enumdef, "TiledEnumDefinition bound")
    ...
    var enumval := TiledEnumValue.new()
    assert_not_null(enumval, "TiledEnumValue bound")
    assert_true(typeof(enumval.contains_value_name("name")) == TYPE_BOOL, "EnumValue contains_value_name exists")
    var tileobj := TiledTileObject.new()
    assert_not_null(tileobj, "TiledTileObject bound")
    var ptypes := TiledProjectPropertyTypes.new()
    assert_not_null(ptypes, "TiledProjectPropertyTypes bound")
func test_exhaustive_method_calls() -> void:

    var class_names = ['TiledTileson', 'TiledLayer', 'TiledTileset', 'TiledWangColor', 'TiledWangTile', 'TiledWangSet', 'TiledFrame', 'TiledAnimation', 'TiledTile', 'TiledObject', 'TiledChunk', 'TiledMap', 'TiledProperty', 'TiledProjectFolder', 'TiledProjectData', 'TiledProject', 'TiledWorldMapData', 'TiledWorld', 'TiledGrid', 'TiledTerrain', 'TiledText', 'TiledTransformations', 'TiledClass', 'TiledEnumDefinition', 'TiledEnumValue', 'TiledTileObject', 'TiledProjectPropertyTypes']
